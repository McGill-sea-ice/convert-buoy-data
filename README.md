...
After `decode_convert.py` is run for the first time, it will create a file `last_access` that saves the last time the gmail account was accessed by the script. The next time `decode_convert.py` is run, it will only convert data newer than the timestamp saved in `last_access`.
